...
export function SeasonStream({
  ...
  // К началу возвращаемся только на решении — и ни на чём другом.
  //
  // Матчи приходят наверх и видны там сами: контейнер стоит на нуле, а если
  // игрок отлистал вниз перечитать прошлый матч, браузер сам удержит его
  // взгляд на месте при вставке сверху. Тащить его к началу каждые семьсот
  // миллисекунд значило бы выдёргивать из чтения ровно за то, что он читает.
  //
  // Решение — другое дело: лента на нём останавливается и будет ждать сколько
  // угодно, но игрок, ушедший вниз, об этом не узнает. Один раз показать ему
  // то, чего от него ждут, честно.
  useEffect(() => {
    if (!view.blocked) return
    topRef.current?.scrollIntoView({ block: 'start', behavior: instant ? 'auto' : 'smooth' })
  }, [view.blocked, instant])
