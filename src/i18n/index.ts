import type { Locale, Localized, Text, TextParam } from '../engine/types'
...
export function formatMoney(value: number, locale: Locale): string {
  ...
  if (abs >= 1_000_000_000) return `${sign}€${round(abs / 1_000_000_000)}${locale === 'ru' ? ' млрд' : 'B'}`
  if (abs >= 1_000_000) return `${sign}€${round(abs / 1_000_000)}${locale === 'ru' ? ' млн' : 'M'}`
  if (abs >= 1_000) return `${sign}€${round(abs / 1_000)}${locale === 'ru' ? ' тыс' : 'K'}`
...
function renderParam(name: string, value: TextParam, locale: Locale): string {
  if (isLocalized(value)) return value[locale]
  if (isText(value)) return t({ key: value.key }, locale)
  if (typeof value === 'number') {
    if (MONEY_PARAMS.has(name)) return formatMoney(value, locale)
...
export function t(text: Text | null | undefined, locale: Locale): string {
  if (!text) return ''
  ...
  return template.replace(/\{(\w+)\}/g, (match, name: string) => {
    ...
    return value === undefined ? match : renderParam(name, value, locale)
  })
}
