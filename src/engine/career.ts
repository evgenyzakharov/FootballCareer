...
/**
 * Границы, между которыми разыгрывается авторитет в новой раздевалке. Именно
 * разыгрывается, а не срезается долей от прежнего: заработанное в старом клубе
 * к новым людям отношения не имеет. Из-за этого первый переход поднимает
 * авторитет вчерашнему воспитаннику — и это не сбой: подписанный игрок и
 * шестнадцатилетний из академии в раздевалке стоят разного.
 */
export const LOCKER_ON_ARRIVAL: [number, number] = [35, 65]
...
function joinClub(state: CareerState, effect: Extract<Effect, { t: 'transfer' }>): CareerState {
  ...
  if (!isSameClub) {
    ...
    // Раздевалка тоже принимает заново, и как именно — лотерея: где-то новичка
    // слушают с первого дня, где-то он снова никто. Заработанное в прежнем
    // клубе на это не влияет вовсе — там его слушали те люди, а не эти.
    next = setGauge(next, 'lockerRoom', rng.int(LOCKER_ON_ARRIVAL[0], LOCKER_ON_ARRIVAL[1]))
    if (next.season) {
