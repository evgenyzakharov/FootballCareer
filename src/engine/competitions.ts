...
export interface CallUpContext {
  ...
  established: boolean
  /**
   * Как игрок отыграл прошлый сезон против того, что от него ждали, —
   * `seasonStanding` из `offers.ts`, та же величина, которой его меряет рынок.
   * Тренер сборной смотрит на то же, что и спортивные директора: OVR говорит,
   * кто игрок есть вообще, сезон — стоит ли звать его этой осенью.
   */
  standing: number
}
...
export function callUpChance(ctx: CallUpContext): number {
  ...
  // Сезон идёт в ту же чашу, что и класс, пункт в пункт: год, отыгранный
  // заметно выше своей задачи, стоит ровно столько же, сколько те же пункты
  // OVR. Иначе в сборную попадали по одной анкете, и лучший бомбардир лиги
  // ждал вызова столько же, сколько запасной с тем же OVR.
  const gap = ctx.ovr + ctx.standing - bar
