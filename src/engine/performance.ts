...
/**
 * Во сколько очков любви трибун обходится полусезон, в котором команда
 * выиграла всё или проиграла всё. Слабее, чем у настроя: настрой — это про то,
 * как человеку живётся, а трибуны судят прежде всего по тому, что игрок делает
 * сам, и главным его каналом остаются голы и передачи. У любви трибун вдобавок
 * нет межсезонного притяжения — она копится всю карьеру и обнуляется только
 * переходом, — поэтому одинаковый с настроем вес разогнал бы её до края шкалы
 * за пару сезонов.
 */
export const FANS_BY_RESULT = 18

/**
 * Личные каналы трибун. Оценка — то же «отыграл на свой уровень», но планка
 * чуть выше, чем у формы: трибуны замечают не старание, а игру. Голы и
 * передачи считаются поштучно и не масштабируются по размеру тура — они и
 * остаются главным, за что игрока любят. Красная карточка стоит дороже гола:
 * оставить команду вдесятером трибуны помнят дольше, чем забитый мяч.
 */
export const FANS_NEUTRAL = 6.85
export const FANS_BY_RATING = 6
export const FANS_BY_GOAL = 0.9
export const FANS_BY_ASSIST = 0.5
export const FANS_BY_RED = 4
...
export function simulateBlock(ctx: BlockContext, rng: Rng): BlockResult {
  ...
  // Настрой и трибуны смотрят не только на свою графу, но и на табло. Хорошо
  // отыграть в команде, проигрывающей каждый второй матч, — это всё равно
  // тяжёлый сезон, и наоборот: победы вытягивают даже того, у кого лично не
  // идёт. Считается только по матчам, в которых игрок был на поле: чужие
  // победы с трибуны никого не греют, а с трибун и судят по тому, что было
  // при нём.
  ...
  // единственное, что удерживает обе шкалы от потолка: карьера в среднем идёт
  ...
  // Трибунам результат нужен не меньше: своим на трибуне становятся в команде,
  // которая выигрывает, а в вылетающей не любят никого. Голы и передачи при
  // этом считаются поштучно и остаются главным личным каналом — забивать в
  // тонущей команде всё же выгоднее, чем не забивать.
  const fanDelta = round(
    ((scored - FANS_NEUTRAL) * FANS_BY_RATING + swing * FANS_BY_RESULT) * part
    + goals * FANS_BY_GOAL + assists * FANS_BY_ASSIST - red * FANS_BY_RED,
    1,
  )
