...
export interface MatchResult {
  ...
  rating: number
  /**
   * Счёт матча глазами команды игрока. Движок считает только самого игрока, но
   * табло нужно и пропущенному матчу: без него пропуск читается как «ничего не
   * было». Полей нет у матчей из сохранений, записанных до их появления, —
   * читать через проверку на undefined.
   */
  teamGoals?: number
  teamConceded?: number
