...
try {
  ...
  for (const [name, width, height, touch] of [['телефон', 375, 812, true], ['планшет', 768, 1024, true], ['десктоп', 1440, 900, false]]) {
    ...
    // Когда блоки складываются в один столбец, лента должна быть первой.
    //
    ...
    // странице, а активная запись при этом уехать за край прокрутки, — поэтому
    // драйвер перед замером сам уводит экран вниз.
    //
    ...
    if (width <= 860) {
      const career = report.screens.career
      check('лента выше полосы игрока', career?.streamBeforeFacts === true, `лента@${career?.stageTop} игрок@${career?.factsTop}`)
      check('лента выше полосы сезона', career?.streamBeforeSeason === true, `лента@${career?.stageTop} сезон@${career?.seasonTop}`)
      check(
