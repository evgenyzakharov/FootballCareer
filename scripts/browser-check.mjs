...
try {
  ...
  for (const [name, width, height, touch] of [['телефон', 375, 812, true], ['планшет', 768, 1024, true], ['десктоп', 1440, 900, false]]) {
    ...
    // Когда блоки складываются в один столбец, лента уезжает в досье первой
    // вкладкой, а сверху остаются шапка, состояние и строка сезона.
    //
    // Здесь же проверяем главное требование к самой ленте: решение живёт в её
    // начале, и игрок, ушедший читать прошлые матчи, обязан его увидеть.
    // Порядком блоков это не проверяется — лента может стоять там, где надо, а
    // активная запись при этом уехать за край прокрутки, — поэтому драйвер
    // перед замером сам уводит экран вниз.
    //
    // Краснеть эта проверка умеет на телефоне: там страницы под две тысячи
    // пикселей при окне в восемьсот, и со сломанным возвратом к началу
    // активная запись уезжает на минус тысячу. На планшете она проходит
    // вхолостую — уводить там некуда, прокрутки набирается пара сотен
    // пикселей, — а на десктопе не запускается вовсе: карьера держится в одном
    // экране, страница не прокручивается, и прокрутку колонки этот драйвер
    // воспроизвести не умеет. Так что настоящее покрытие тут одно, узкое; знать
    // об этом важнее, чем иметь три зелёных строчки.
    if (width <= 860) {
      const career = report.screens.career
      check('лента живёт во вкладке досье', career?.streamInDossier === true, `лента@${career?.stageTop} досье@${career?.dossierTop}`)
      check('полоса сезона свёрнута в шапку', career?.seasonMerged === true, `сезон@${career?.seasonTop}`)
      check('шапка выше досье', career?.factsBeforeDossier === true, `игрок@${career?.factsTop} досье@${career?.dossierTop}`)
      // Закрепление меряется после ухода вниз: драйвер уводит экран сам, и
      // незакреплённая шапка уехала бы в минус вместе со страницей.
      check('шапка закреплена под верхней панелью', career?.factsUnderTopbar === true, `верх шапки ${career?.factsWindowTop}`)
      // Закреплённая шапка со шкалами занимала шестьдесят процентов экрана.
      // Уйдя вниз по ленте, она их отдаёт — иначе закрепление стоит дороже
      // того, ради чего оно заводилось.
      check(
      ...
      check('свёрнутая шапка не занимает экран', career?.factsHeight !== null && career?.factsHeight < height * 0.5, `${career?.factsHeight}px при ${height}`)
      check(
        'решение видно после ухода вниз по ленте',
        career?.liveVisible === true,
        `верх ${career?.liveTop} при высоте окна ${height}`,
      )
    }
